...
// void Block::gradientDescent(const Eigen::MatrixXd& x_batch, const Eigen::MatrixXd& u_batch, size_t
// batch_size)
// {
...
BlackBox::BlackBox(size_t blocks_cnt, std::ifstream& settings)
    : blocks_cnt(blocks_cnt), blocks(std::vector<std::unique_ptr<Block> >(blocks_cnt)) {
    size_t in_dim;
    size_t out_dim;
...
//     for (int i = 1; i < blocks_cnt; ++i) {
//         remember_output.emplace_back(std::make_unique<Eigen::MatrixXd>(blocks[i]->evaluate(*remember_output[i
//         - 1])));
//     }
