...
public:
    ...
    void loadTrainCSV(const std::filesystem::path& path, std::vector<int> target_cols_idx, char delimiter = ',', bool have_header = false, int batch_size = 32);

    double loadTestCSV(const std::filesystem::path& path, std::vector<int> target_cols_idx, std::function<bool(const Vector&, const Vector&)> is_correct, char delimiter = ',', bool have_header = false);
